...
public:
	...
	void SetGlobResults(vector<OpenFileInfo> results) {
...
TEST_CASE("Test Glob with GlobFilesExtended unsupported", "[glob test]") {
	...
	mock_filesystem->SetGlobResults(std::move(glob_results));

	auto wrapper = make_uniq<FileSystemTimeoutRetryWrapper>(std::move(mock_filesystem), db_instance);
	auto results = wrapper->Glob("s3://bucket/snapshots/*.parquet");
